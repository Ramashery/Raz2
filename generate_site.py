...
# ИЗМЕНЕНИЕ: Загружаем оба шаблона
...
# ИЗМЕНЕНИЕ: Новая функция для генерации главной страницы
def generate_home_page(all_data):
    """Генерирует главную страницу index.html из шаблона home_template.html."""
    try:
...
def generate_detail_page(item, all_data):
    ...
    # ИЗМЕНЕНИЕ: Путь теперь всегда включает язык, даже 'en' для единообразия
    lang_prefix = f"{lang}/"
...
# ИЗМЕНЕНИЕ: Функция копирования ассетов вынесена отдельно
def copy_static_assets():
    ...
    ignore_list = {
        ...
        'template.html', 'home_template.html', 'firebase.json', 
        'README.md', '__pycache__', 'index.html', 'page.html', 'admin.txt', 'main.txt'
    }
...
def generate_sitemap_xml(pages_for_sitemap):
    ...
    SubElement(url_el, 'xhtml:link', rel="alternate", hreflang="x-default", href=f"{BASE_URL}/en/")
...
def main():
    ...
    # ИЗМЕНЕНИЕ: Новый порядок действий
    # 1. Генерируем все HTML страницы
    ...
    # 2. Копируем статические ассеты (CSS, JS и т.д.)
    copy_static_assets()
