...
# Настройки для Sitemap (теперь используются как резервный вариант)
SITEMAP_CONFIG = {
...
# --- ФУНКЦИИ (без изменений до generate_sitemap_xml) ---
...
def build_url_for_sitemap(page):
    ...
    slug = page['urlSlug']
    # Убедимся, что слэш всегда есть в конце
...
def generate_sitemap_xml(pages_for_sitemap):
    ...
    for home_url in home_urls:
        ...
        SubElement(url_el, 'loc').text = home_url
        ...
        # Добавляем ссылки на все альтернативные версии
        SubElement(url_el, 'xhtml:link', rel="alternate", hreflang="x-default", href=f"{BASE_URL}/")
        SubElement(url_el, 'xhtml:link', rel="alternate", hreflang="en", href=f"{BASE_URL}/")
        SubElement(url_el, 'xhtml:link', rel="alternate", hreflang="ka", href=f"{BASE_URL}/ka/")
        SubElement(url_el, 'xhtml:link', rel="alternate", hreflang="ru", href=f"{BASE_URL}/ru/")
        SubElement(url_el, 'xhtml:link', rel="alternate", hreflang="ua", href=f"{BASE_URL}/ua/")
    ...
    for group_key, pages_in_group in grouped_pages.items():
        # Создаем карту ссылок для данной группы (en -> url, ru -> url)
        ...
        for page in pages_in_group:
            ...
            # Используем новые данные из Firebase с резервным вариантом
            ...
            # Добавляем xhtml:link теги для всей группы
    ...
    for page in single_pages:
        ...
        priority = page.get('sitemapPriority', default_config['priority'])
        SubElement(url_el, 'priority').text = str(priority)
        # Для одиночных страниц xhtml:link не добавляем

    # --- 5. Сохранение файла ---
    xml_string = tostring(urlset, 'utf-8')
    pretty_xml = minidom.parseString(xml_string).toprettyxml(indent="    ") # Используем 4 пробела для отступа, как в образце
