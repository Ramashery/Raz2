...
def copy_static_assets():
    """Копирует все необходимые статические файлы (CSS, JS, изображения и т.д.) в папку public."""
...
# ==============================================================================
# === НОВАЯ, УЛУЧШЕННАЯ ФУНКЦИЯ ГЕНЕРАЦИИ SITEMAP.XML С HREFLANG ===
# ==============================================================================
def generate_sitemap_xml(pages_for_sitemap):
    # --- НАЧАЛО: Неизменная часть для создания XML-документа ---
    urlset = Element('urlset', xmlns="http://www.sitemaps.org/schemas/sitemap/0.9")
    urlset.set('xmlns:xhtml', "http://www.w3.org/1999/xhtml")
    today = date.today().isoformat()
    # --- КОНЕЦ: Неизменная часть ---


    # === ШАГ 1: Генерация блоков для ГЛАВНОЙ страницы ===
    # Создаем полный набор hreflang ссылок для главной страницы
    home_hreflangs = {lang: f"{BASE_URL}/{lang}/" for lang in SUPPORTED_LANGS}
    home_hreflangs['en'] = f"{BASE_URL}/" # Английская версия на корневом домене
    home_hreflangs['x-default'] = f"{BASE_URL}/" # Версия по умолчанию

    # Создаем <url> блок для КАЖДОЙ языковой версии главной страницы
    for lang in SUPPORTED_LANGS:
        url_el = SubElement(urlset, 'url')
        loc = home_hreflangs[lang] # URL этой конкретной версии
        ...
        # В каждый блок добавляем ПОЛНЫЙ набор ссылок на все языковые версии
        for lang_code, href in home_hreflangs.items():
            SubElement(url_el, 'xhtml:link', rel="alternate", hreflang=lang_code, href=href)


    # === ШАГ 2: Группировка всех остальных страниц по ключу перевода ===
    grouped_pages = {}
    for page in pages_for_sitemap:
        # Используем translationGroupKey для группировки. Если его нет, используем ID как запасной вариант.
    ...
    # === ШАГ 3: Генерация блоков для сгруппированных страниц ===
    # Проходим по каждой группе страниц (например, по всем переводам одной статьи блога)
    for group_key, pages_in_group in grouped_pages.items():
        
        # Сначала создаем полный набор hreflang ссылок для ЭТОЙ группы
        hreflang_map = {p['lang']: build_url_for_sitemap(p) for p in pages_in_group}
        
        # Определяем x-default для группы (английская версия, если есть, иначе первая попавшаяся)
        if 'en' in hreflang_map:
            hreflang_map['x-default'] = hreflang_map['en']

        # Теперь создаем <url> блок для КАЖДОЙ страницы внутри группы
        for page in pages_in_group:
            ...
            # В каждый блок добавляем ПОЛНЫЙ набор ссылок на все языковые версии из этой группы
    ...
    # --- НАЧАЛО: Неизменная часть для сохранения файла ---
    xml_string = tostring(urlset, 'utf-8')
    pretty_xml = minidom.parseString(xml_string).toprettyxml(indent="    ")
    output_path = os.path.join(OUTPUT_DIR, 'sitemap.xml')
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(pretty_xml)
    print(f"✓ Карта сайта (с hreflang) успешно сгенерирована: {output_path}")
