...
try:
    if not firebase_admin._apps:
        # Пытаемся взять ключ из переменной окружения (для GitHub) или из файла (локально)
...
def format_content(content_string):
    ...
    for block in blocks:
        ...
        if yt_match:
            ...
            # ИСПРАВЛЕНИЕ: Избегаем обратного слеша внутри f-строки для совместимости
...
def generate_pages(data):
    ...
    for col in ['services', 'portfolio', 'blog', 'contact']:
        for item in data[col]:
            ...
            lang = item['lang']
            lang_dir = lang if lang != 'en' else 'en'
...
def copy_assets():
    ignore = {'.git', OUTPUT_DIR, 'generate_site.py', 'template.html', 'index_template.html', 'firebase.json', 'serviceAccountKey.json', '.github'}
