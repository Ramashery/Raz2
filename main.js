...
// --- SEO & DATA FUNCTIONS (без изменений) ---
...
function applyCustomBackground(item = null) { const iframe = document.getElementById('custom-background-iframe'); const customCode = item?.backgroundHtml || siteData.home?.backgroundHtml || ''; if (customCode && customCode.trim() !== "") { if (iframe.style.display !== 'block') iframe.style.display = 'block'; if (iframe.srcdoc !== customCode) iframe.srcdoc = customCode; } else { if (iframe.style.display !== 'none') iframe.style.display = 'none'; iframe.srcdoc = ''; } }
...
function routeAndRender(isPopState = false) {
    ...
    // Распределяем классы анимации
    document.querySelectorAll('.detail-content > *').forEach(el => el.classList.add('floating-item'));
    document.querySelectorAll('.detail-price, #related-posts .item-card, #services .animated-container, #portfolio .animated-container, #blog .animated-container, #contact .animated-container, #services .item-card, #portfolio .item-card, #blog .item-card, #contact .item-card').forEach(el => el.classList.add('animate-on-scroll'));

    // Запускаем наблюдателей для новой страницы
    setupObservers();
}
...
function handleNavigation(e) {
    ...
    // ============================================
    // ОБРАБОТКА ЯКОРНЫХ ССЫЛОК (HASH)
    // ============================================
    if (targetUrl.hash) {
        const menuCloseDelay = isMenuOpen ? 350 : 0;

        setTimeout(() => {
            // СЛУЧАЙ 1: Якорь на той же странице (/#services -> /#portfolio)
            ...
            // СЛУЧАЙ 2: Переход с другой страницы на главную с якорем
            ...
            setTimeout(() => {
                // Переходим на целевую страницу БЕЗ hash
                window.history.pushState({}, '', targetUrl.pathname);
                
                // Отключаем transition для мгновенного рендера
                ...
                requestAnimationFrame(() => {
                    requestAnimationFrame(() => {
                        // Страница отрисована, возвращаем transition
                        mainContentEl.style.transition = 'opacity 0.4s ease-in-out, transform 0.4s ease-in-out';
                        ...
                        // Еще один двойной RAF для гарантии, что обсерверы отработали
                        requestAnimationFrame(() => {
                            requestAnimationFrame(() => {
                                const targetElement = document.getElementById(targetUrl.hash.substring(1));
                                if (targetElement) {
                                    // Скроллим с небольшой задержкой для плавности
                                    setTimeout(() => {
                                        targetElement.scrollIntoView({ behavior: 'smooth' });
                                    }, 50);
                                } else {
                                    // Если элемент не найден, скроллим наверх
    ...
    // ============================================
    // ОБРАБОТКА ОБЫЧНЫХ ССЫЛОК (БЕЗ HASH)
    // ============================================
    
    // Клик на ту же страницу - скролл наверх
...
function initStaticEventListeners() {
    ...
    document.getElementById('site-footer').innerHTML = `© ${new Date().getFullYear()} Digital Craft. All rights reserved.`;
}
