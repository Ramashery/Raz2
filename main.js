...
// Глобальная переменная для контента (инициализируется в initApp)
let mainContentEl;
    ...
    if (targetUrl.hash) { 
        ...
        setTimeout(() => { 
            if (targetUrl.pathname === window.location.pathname) { 
                ...
                if (targetElement) { window.history.pushState({}, '', targetUrl.href); targetElement.scrollIntoView({ behavior: 'smooth' }); } 
                return; 
            } 
                ...
                requestAnimationFrame(() => { 
                    requestAnimationFrame(() => { 
                        ...
                        requestAnimationFrame(() => { 
                            requestAnimationFrame(() => { 
                                const targetElement = document.getElementById(targetUrl.hash.substring(1)); 
                                if (targetElement) { setTimeout(() => targetElement.scrollIntoView({ behavior: 'smooth' }), 50); } 
...
async function hydrateStaticPage() {
    ...
    } catch (error) { console.error("Background data load failed during hydration:", error); }
}
...
async function initApp() { 
    // Инициализируем ссылку на main здесь
    ...
    if (isStaticPage) { 
        ...
        try { 
            ...
            renderMenu(); 
            routeAndRender(); 
            if (mainContentEl) mainContentEl.classList.remove('loading'); 
            const loader = document.getElementById('loader'); 
