...
async function initApp() {
    ...
    try {
        ...
        renderMenu();
        renderHomePage(); // Отрисовываем всё из базы
...
function renderHomePage() {
    ...
    // 1. Восстанавливаем HERO в точности по вашей верстке
    const heroSection = document.getElementById('hero');
    heroSection.innerHTML = `
        <h1 class="animate-always is-visible">${home.h1 || ''}</h1>
    ...
    // 3. Оживляем слайдеры и анимации
...
function renderSection(key, title, items) {
    ...
    // Верстка для Десктопа (Грид-слайды по 3 карточки)
    ...
    // Верстка для Мобайла (Cross-fade слайдер)
