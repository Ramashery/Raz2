...
let floatingObserver, animateOnceObserver, animateAlwaysObserver, heroObserver;

// --- OBSERVERS ---
function setupObservers() {
    // Отключаем старые, чтобы не плодить дубликаты
    ...
    // 1. Observer для обычных элементов (Services, Portfolio...) - срабатывает один раз
    ...
    // 2. Observer для плавающих элементов (детали) - работает туда-сюда для параллакс эффекта
    ...
    // 3. НОВЫЙ Observer для Hero (Бесконечная анимация)
    heroObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // Элемент виден - добавляем класс, запускающий анимацию
                entry.target.classList.add('in-view');
            } else {
                // Элемент ушел - удаляем класс, сбрасывая анимацию в исходное состояние
                entry.target.classList.remove('in-view');
            }
        });
    }, { threshold: 0.1 }); 

    // 4. Обычный animate-always для прочих элементов (если есть)
    ...
    // Подключаем элементы к обсерверам
    ...
    // Подключаем Hero к новому обсерверу
    document.querySelectorAll('.hero-animate').forEach(el => heroObserver.observe(el));
}
...
function initMobileSliders() {
    document.querySelectorAll('.language-slider-block').forEach(sliderBlock => {
        ...
        slider.addEventListener('touchend', e => {
            ...
            if (touchEndX < touchStartX - 40) goToSlide(currentIndex + 1);
            else if (touchEndX > touchStartX + 40) goToSlide(currentIndex - 1);
            startAutoSlide();
        }, { passive: true });
    });
}
...
function routeAndRender(isPopState = false) {
    ...
    const animateSelector = '.detail-price, #related-posts .item-card, #services .animated-container, #portfolio .animated-container, #blog .animated-container, #contact .animated-container, #services .item-card, #portfolio .item-card, #blog .item-card, #contact .item-card';
    document.querySelectorAll(animateSelector).forEach(el => el.classList.add('animate-on-scroll'));
...
async function hydrateStaticPage() {
    console.log("Hydrating static page (CSS Hero + Re-animate)...");
    renderMenu();
    setupObservers();
    
    document.querySelectorAll('.detail-price').forEach(el => el.classList.add('animate-always', 'is-visible'));
    ...
    try {
        ...
        if (match) {
            ...
            if (currentItem) {
                ...
                if (!document.getElementById('related-posts')) renderRelatedPosts(collection, slug, itemLang);
        ...
        console.error("Data load failed during hydration:", error);
    }
}
