...
function handleNavigation(e) {
    ...
    if (targetUrl.pathname === window.location.pathname && targetUrl.hash) {
        e.preventDefault();
        document.getElementById(targetUrl.hash.substring(1))?.scrollIntoView({ behavior: 'smooth' });
        return;
    }
    ...
    setTimeout(() => {
        ...
        mainContentEl.style.transition = 'none'; // Временно отключаем transition для мгновенной отрисовки
        routeAndRender();
        ...
        }, 20); // Микро-задержка для браузера
...
async function initApp() {
    ...
    try {
        ...
        renderMenu();
        routeAndRender();
        // Убираем класс loading после первого рендера, чтобы запустить анимацию
