...
async function hydrateStaticPage() {
    ...
    // Анимации для уже существующего контента
    ...
    setupObservers(); // Запускаем наблюдатели для контента, который уже есть на странице

    try {
        siteData = await loadData(); // Загружаем все данные в фоне
        const path = window.location.pathname;
        const detailPageRegex = /^\/(?:([a-z]{2})\/)?(services|portfolio|blog|contact)\/([a-zA-Z0-9-]+)\/?$/;
        const match = path.match(detailPageRegex);

        if (match) {
            // Логика для ВНУТРЕННИХ страниц (гидратация фона и т.д.)
            ...
            if (currentItem) {
                ...
                // Если вдруг related-posts не сгенерировались, JS их добавит
            ...
            // Добавляем классы анимации к новым элементам
            ...
            // Повторно запускаем observers, чтобы они "увидели" новые элементы
            setupObservers();
