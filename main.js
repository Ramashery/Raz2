...
// ИЗМЕНЕНИЕ: Старая функция renderHomePage() удалена.
...
// ИЗМЕНЕНИЕ: Новая функция для навигации на главную
async function navigateToHome() {
    try {
        ...
        if (newMain) {
            ...
            // Запускаем ту же логику гидратации, что и при первой загрузке
            hydrateHomePageContent();
        }
    } catch (error) {
        console.error("Failed to navigate to home, reloading:", error);
        window.location.href = '/'; // В случае ошибки, просто перезагружаем страницу
    }
}
...
function routeAndRender(isPopState = false) { 
    ...
    if (match) { 
        ...
        // ИЗМЕНЕНИЕ: Вызываем новую функцию вместо старой renderHomePage()
        navigateToHome();
    } 
...
// ИЗМЕНЕНИЕ: Логика гидратации главной вынесена в отдельную функцию для переиспользования
...
async function hydrateStaticPage() {
    ...
    try {
        ...
        if (match) {
            ...
            // ИЗМЕНЕНИЕ: Вызываем новую общую функцию
            hydrateHomePageContent();
        }
    } catch (error) {
...
async function initApp() { 
    ...
    if (isStaticPage) { 
        ...
        // Этот блок больше не используется для SSG, но оставлен на всякий случай
        try { 
            siteData = await loadData(); 
            renderMenu(); 
            routeAndRender(); 
