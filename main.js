...
// Глобальные переменные для кнопки прокрутки и отслеживания типа страницы
let scrollButton;
let isCurrentPageDetailPage = false; // Флаг, указывающий, является ли текущая страница детальной
...
function renderDetailPage(collection, slug, lang) {
    console.log(`Rendering detail page: ${collection}/${slug} in ${lang}`);
    const item = siteData[collection]?.find(d => d.urlSlug === slug && d.lang === lang);
    if (!item) {
        console.error(`Item not found for detail page: ${collection}/${slug} (${lang})`);
...
function calculateQuizResult() {
    ...
    if (!resultDiv) {
        console.error('Quiz result container not found!');
        return;
    }
    ...
    if (scores.hybrid >= 5 || (scores.hybrid >= scores.landing && scores.hybrid >= scores.website)) {
        resultHTML = `
            <h3>Your Result: Hybrid Approach</h3>
            <p>Consider a phased approach: start with a core website (5-8 pages) and add campaign-specific landing pages as needed. This balances long-term brand building with short-term conversion optimization.</p>
        `;
        ...
        resultHTML = `
            <h3>Your Result: Landing Page</h3>
            <p>You need a focused, conversion-optimized landing page. Perfect for campaigns, single offers, events, or testing new business ideas. Expected investment: $500-$2,000 for professional landing page design in Tbilisi.</p>
        `;
        resultStyle = 'background-color: rgba(230, 247, 255, 0.1); border-color: #91d5ff;';
    } else {
        resultHTML = `
            <h3>Your Result: Multi-Page Website</h3>
            <p>You need comprehensive site development. Invest in a multi-page website to build brand credibility, rank in search engines, and support complex sales processes. Expected investment: $3,000-$15,000+ depending on features.</p>
        `;
...
function toggleScrollButtonVisibility() {
    ...
    // Ищем первый H1 только в пределах основного контента
    const firstH1 = document.querySelector('main h1'); 

    // Кнопка должна быть видна только на детальных страницах,
    // если пользователь прокрутил вниз на 200px (или больше)
    // И если на странице есть H1, к которому можно прокрутить
    if (isCurrentPageDetailPage && window.scrollY > 200 && firstH1) {
...
// Функция для прокрутки к первому H1 на странице
function scrollToH1() {
    const firstH1 = document.querySelector('main h1');
    if (firstH1) {
...
async function navigateToHome() {
    ...
    isCurrentPageDetailPage = false; // Это домашняя страница
    ...
    try {
        ...
        if (newMain) {
            ...
            // mainContentEl.classList.remove('loading'); // Удалим в finally initApp
            ...
            setTimeout(() => {
                ...
                if (heroSubtitle) {
                    heroSubtitle.classList.remove('is-visible');
                    heroSubtitle.offsetHeight;
                    setTimeout(() => heroSubtitle.classList.add('is-visible'), 200);
                }
            }, 50);
...
async function routeAndRender(isPopState = false) {
    console.log(`Routing and rendering for path: ${window.location.pathname} (isPopState: ${isPopState})`);
    ...
    if (match) {
        isCurrentPageDetailPage = true; // Это детальная страница
        const [, lang, collection, slug] = match;
        renderDetailPage(collection, slug, lang || 'en');
    } else {
        isCurrentPageDetailPage = false; // Это не детальная страница (скорее всего, домашняя)
    ...
    // Обновить видимость кнопки после рендеринга и установки флага isCurrentPageDetailPage
...
function hydrateHomePageContent() {
    console.log("Hydrating home page content.");
    ...
    if (preloadedDataEl) {
        try {
            ...
            console.log("✓ Используются предзагруженные данные из HTML");
            ...
            console.error("Ошибка при парсинге предзагруженных данных:", e);
        }
    }
    ...
    isCurrentPageDetailPage = false; // При гидрации домашней страницы, устанавливаем флаг
    toggleScrollButtonVisibility(); // Убеждаемся, что кнопка скрыта на главной
}
...
async function hydrateStaticPage() {
    ...
    try {
        ...
        if (match) {
            ...
            isCurrentPageDetailPage = true; // Для статических detail страниц устанавливаем флаг
        } else {
            // Если static-page=true, но URL не детальный, это странная ситуация,
            // но на всякий случай обрабатываем как домашнюю (чтобы что-то показать)
            hydrateHomePageContent();
        }
    } catch (error) {
        console.error("Background data load failed during hydration:", error);
    }
    // Обновить видимость кнопки после гидрации статической страницы
    toggleScrollButtonVisibility();
}
...
function initStaticEventListeners() { 
    ...
    // === ДОБАВЛЕННЫЙ КОД ДЛЯ ОБРАБОТКИ КЛИКА ===
    mainContentEl.addEventListener('click', function(event) {
        if (event.target.id === 'quiz-submit-button') {
            calculateQuizResult();
        }
    });
    ...
    if (scrollButton) {
        window.addEventListener('scroll', toggleScrollButtonVisibility); // Отслеживание прокрутки
        scrollButton.addEventListener('click', scrollToH1); // Обработчик клика
    }
}
...
async function initApp() { 
    ...
    try {
        ...
        mainContentEl.innerHTML = `<h1>Error loading site data. Please try again later.</h1><p>${error.message}</p>`; 
    } finally {
        // ================== ИЗМЕНЕНИЕ ЗДЕСЬ ==================
        // Гарантируем удаление класса 'loading' в конце инициализации,
        // чтобы контент стал видимым, даже если были ошибки.
        // Если main.loading:has(h1) в styles.css был удален, это не критично.
        // Если он остался, то h1 может быть виден раньше, но основной контейнер будет виден в любом случае.
        if (mainContentEl.classList.contains('loading')) {
            console.log("Removing 'loading' class from main content element.");
