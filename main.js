...
// --- HELPER: Reanimate Hero (Moved from index.html) ---
function reanimateHero() {
    const heroH1 = document.querySelector('.hero h1');
    const heroSubtitle = document.querySelector('.hero-subtitle-container');
    
    if (!heroH1 || !heroSubtitle) return;
    
    heroH1.classList.remove('is-visible');
    heroSubtitle.classList.remove('is-visible');
    
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            heroH1.classList.add('animate-always');
            heroSubtitle.classList.add('animate-always');
            setTimeout(() => {
...
function renderSeoTags(data) {
    ...
    // Логика обработки слешей для канонических ссылок
...
function routeAndRender(isPopState = false) {
    ...
    if (match) {
        ...
        renderHomePage();
        // Встроенная логика reanimateHero
        setTimeout(() => reanimateHero(), 100);
    }
...
async function hydrateStaticPage() {
    ...
    try {
        ...
        if (match) {
            ...
            // Если это главная страница, применяем фон из данных
            applyCustomBackground(siteData.home);
            initMobileSliders();
            initDesktopCarousels();
        }
