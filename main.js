...
function renderSeoTags(data) {
    ...
    if (typeof schemaData === 'string' && schemaData.trim()) {
        try { schemaData = JSON.parse(schemaData); } catch (e) { console.error('Failed to parse schemaJsonLd:', e); schemaData = null; }
    }
    if (schemaData && typeof schemaData === 'object' && Object.keys(schemaData).length > 0) {
        ...
        const fallbackSchema = generateFallbackSchema(data);
        if (fallbackSchema) {
            const script = document.createElement('script');
            script.type = 'application/ld+json';
            script.textContent = JSON.stringify(fallbackSchema);
            document.head.appendChild(script);
        }
    }
}
...
function hydrateStaticPage(siteData) {
    ...
    if (item) {
        console.log("Hydrating static page with fresh data:", item.title);
...
function renderHomePage() { 
    ...
    if (!heroExists) {
        ...
        <section id="hero" class="hero">
            <h1>Web Development & SEO in Tbilisi</h1>
        ...
        <noscript>
            <section>
                <h2>Our Services</h2>
                <ul>
                    <li><a href="/en/services/custom-web-development-tbilisi/">Web Development</a></li>
                    <li><a href="/en/services/seo-optimization-tbilisi/">SEO Optimization</a></li>
                </ul>
            </section>
        </noscript>
        `; 
...
function renderRelatedPosts(currentCollection, currentSlug, currentLang) { 
    ...
    const relatedSection = document.createElement('section'); relatedSection.id = 'related-posts'; relatedSection.innerHTML = `<h2 class="fade-in-up">You Might Also Like</h2><div class="item-grid">${itemsHTML}</div>`; mainContentEl.appendChild(relatedSection); initFloatingObservers(); 
}
...
function renderSection(key, title, items) { 
    ...
    const mobileContainer = `<div class="mobile-sliders-container">${mobileSlidersHTML}</div>`; section.innerHTML = `<div class="animated-container"><h2>${title}</h2></div>${desktopWrapper}${mobileContainer}`; 
};
...
function initHomePageObservers() { 
    ...
    homePageObserver = new IntersectionObserver((entries) => { 
        entries.forEach(entry => { 
            const animatedElements = entry.target.querySelectorAll('.animated-container'); 
    ...
    document.querySelectorAll('#services, #portfolio, #blog, #contact, #related-posts').forEach(section => { 
        if(section) homePageObserver.observe(section); 
    }); 
}


// *** ОКОНЧАТЕЛЬНАЯ, НАДЕЖНАЯ ВЕРСИЯ ФУНКЦИИ НАВИГАЦИИ ***
function handleNavigation(e) {
    ...
    // Закрываем меню при любом навигационном клике
    ...
    // Сценарий 1: Клик по якорной ссылке на той же странице (мы на "/" и кликаем "/#services")
    if (isHomePage && linkUrl.pathname === '/' && linkUrl.hash) {
        e.preventDefault();
        document.getElementById(linkUrl.hash.substring(1))?.scrollIntoView({ behavior: 'smooth' });
        return;
    }

    // Сценарий 2: Клик по ссылке на ту же страницу, где мы уже находимся (без якоря)
    if (linkUrl.pathname === window.location.pathname && !linkUrl.hash) {
        e.preventDefault();
        window.scrollTo({ top: 0, behavior: 'smooth' }); // Просто плавно скроллим вверх
        return;
    }

    // Сценарий 3: Все остальные случаи (переход на другую страницу)
    e.preventDefault();
    
    // Если мы переходим на ту же страницу, но с другим якорем, не нужно перерисовывать
    if (linkUrl.pathname === window.location.pathname) {
         window.history.pushState({}, '', linkUrl.href);
    } else {
        window.history.pushState({}, '', linkUrl.href);
        routeAndRender();
    }

    // Используем requestAnimationFrame для надежной прокрутки ПОСЛЕ рендеринга
    requestAnimationFrame(() => {
        if (linkUrl.hash) {
            const targetElement = document.getElementById(linkUrl.hash.substring(1));
            if (targetElement) {
                targetElement.scrollIntoView({ behavior: 'smooth' });
            }
        } else {
            // Если якоря нет, скроллим в самый верх
...
function initApp() {
    ...
    if (isStaticPage) {
        console.log("Static page detected. Hydrating...");
